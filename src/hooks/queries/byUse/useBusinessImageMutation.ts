...
export function useBusinessImageActions(bucketName: string, folderName: string, userId: string, groupId: string) {
  const queryClient = useQueryClient();
  const { setImages, deleteImage, updateImage, images } = useImageUploadStore();
  const currentDate = new Date().toISOString();
  const uploadSessionId = uuidv4();

  // 이미지 업로드
  const uploadBusinessImage = useMutation({
  ...
  // 이미지 삭제
  const deleteBusinessImage = useMutation({
  ...
  // 대표 이미지 설정
  const coverBusinessImage = useMutation({
